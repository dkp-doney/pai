...
public class Valuemapper extends MapReduceBase implements Mapper<LongWritable, Text, Text, MapWritable> {
    ...
    public void map(LongWritable key, Text value,OutputCollector<Text, MapWritable> output, Reporter reporter) throws IOException{
        ...
        try{System.out.println("pass1 iin m");
            mw.put(new IntWritable(1),
                    new IntWritable(Integer.parseInt(lineArr[5])));
            mw.put(new IntWritable(2),
                    new IntWritable(Integer.parseInt(lineArr[6])));
            mw.put(new IntWritable(3),
                    new IntWritable(Integer.parseInt(lineArr[7])));
...
//            System.out.println(op.get());
            output.collect(new Text(wk),new MapWritable(mw));
        }
        catch (Exception e){
